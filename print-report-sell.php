...
// توابع تبدیل تاریخ (برای جلوگیری از خطای undefined function)
...
if (!$work_month_id) die('ماه کاری انتخاب نشده است.');
...
if (!$month) die('ماه کاری یافت نشد.');
...
// لیست دقیق 157 محصول (به ترتیب دلخواه تو)
$fixed_products = [
    'آبرسان نیتروژنار',
    'آکواژل لاروشه',
    'اسپری بریدگی/سوختگی',
...
// گرفتن فروش‌ها
$sales_query = "
    SELECT oi.product_name, SUM(oi.quantity) as total_quantity, SUM(oi.total_price) as total_price, p.unit_price
    FROM Order_Items oi
...
foreach ($fixed_products as $product_name) {
    if (isset($sales_map[$product_name])) {
        $sale = $sales_map[$product_name];
        $report[] = [
    ...
    <script>
        function saveReportAsPNG() {
            ...
            for (let i = 1; i <= totalPages; i++) {
                html2canvas(document.getElementById('page-' + i), {scale: 3}).then(canvas => {
