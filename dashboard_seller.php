...
$partners_today = $stmt_partners->fetch(PDO::FETCH_ASSOC); // فقط یه ردیف می‌خواهیم
...
if ($current_start_month && $current_end_month && preg_match('/^\d{4}-\d{2}-\d{2}$/', $current_start_month) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $current_end_month)) {
    ...
    foreach ($date_range as $date) {
        ...
        if ($day_name === $persian_day) {
            $jalali_date = gregorian_to_jalali_format($work_date) . " ($day_name)";
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive" style="overflow-x: auto; width: 100%;">
                        <table id="topProductsTable" class="table table-light table-hover display nowrap"
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="btn-group mb-3" role="group">
                        <button type="button" class="btn btn-primary active" id="dailyBtn"
                            onclick="showDailyChart()">روزانه</button>
                        <button type="button" class="btn btn-primary" id="weeklyBtn"
                            onclick="showWeeklyChart()">هفتگی</button>
                        <button type="button" class="btn btn-primary" id="monthlyBtn"
                            onclick="showMonthlyChart()">ماهانه</button>
                    </div>
                    <canvas id="salesChart"></canvas>
                    <div class="mt-3">
                        <p>رشد امروز نسبت به <?= $persian_day ?> قبلی: <span
                                style="color: <?= $growth_today_color ?>"><?= $growth_today_sign ?><?= number_format(abs($growth_today), 0) ?>
                                تومان</span></p>
                        <p>رشد این ماه نسبت به ماه کاری قبلی: <span
                                style="color: <?= $growth_month_color ?>"><?= $growth_month_sign ?><?= number_format(abs($growth_month), 0) ?>
                                تومان</span></p>
                    </div>
                </div>
            </div>
