...
if ($selected_work_month_id) {
    ...
    if ($month) {
        ...
        $details_query .= " ORDER BY wd.work_date DESC";
...
$orders_query .= " ORDER BY wd.work_date DESC LIMIT " . (int) $per_page . " OFFSET " . (int) $offset;
...
<div class="container-fluid">
            ...
            <select name="work_day_id" class="form-select" onchange="this.form.submit()">
                ...
                <?php foreach ($work_details as $day): ?>
                    <option value="<?= $day['work_details_id'] ?>" <?= $selected_work_day_id == $day['work_details_id'] ? 'selected' : '' ?>>
                        <?= gregorian_to_jalali_format($day['work_date']) ?> (<?= $day['work_day'] ?> - <?= $day['user1'] ?> - <?= $day['user2'] ?>)
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
    ...
    <?php if (!$is_admin && !empty($work_details)): ?>
        <div class="mb-3">
            <?php foreach ($work_details as $day): ?>
                <a href="add_order.php?work_details_id=<?= $day['work_details_id'] ?>" 
                   class="btn btn-primary me-2 mb-2">
                    ثبت سفارش جدید (<?= gregorian_to_jalali_format($day['work_date']) ?> - <?= $day['user1'] ?> - <?= $day['user2'] ?>)
                </a>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
