...
// لود آیتم‌های سفارش
try {
    $stmt = $pdo->prepare("SELECT toi.*, p.product_name 
    ...
    if (empty($items)) {
        ...
        echo "<div class='container-fluid mt-5'><div class='alert alert-warning text-center'>هیچ محصولی برای این سفارش یافت نشد. لطفاً سفارش را بررسی کنید.</div></div>";
        require_once 'footer.php';
        exit;
    }

    // تنظیم item_index و اطمینان از ساختار صحیح
    foreach ($items as &$item) {
        $item['item_index'] = (int) $item['item_index'];
        $item['product_name'] = $item['product_name'] ?? 'محصول نامشخص';
        $item['quantity'] = (int) $item['quantity'];
        $item['unit_price'] = (float) $item['unit_price'];
        $item['extra_sale'] = (float) $item['extra_sale'];
        $item['total_price'] = (float) $item['total_price'];
    }
    unset($item);
} catch (PDOException $e) {
    error_log("Error fetching temp order items: " . $e->getMessage());
    echo "<div class='container-fluid mt-5'><div class='alert alert-danger text-center'>خطا در دریافت آیتم‌های سفارش: " . htmlspecialchars($e->getMessage()) . "</div></div>";
    require_once 'footer.php';
    exit;
}
...
<script>
    ...
    function renderItemsTable(data) {
        ...
        const postalPrice = Number(data.postal_price) || 50000;
        const discount = Number(data.discount) || 0;

        // بررسی خالی بودن آیتم‌ها
        if (items.length === 0) {
            itemsTable.innerHTML = '<p class="text-center">هیچ محصولی در سفارش وجود ندارد.</p>';
            totalAmountDisplay.textContent = '0 تومان';
            finalAmountDisplay.textContent = (postalEnabled ? postalPrice : 0).toLocaleString('fa') + ' تومان';
            postalPriceDisplay.textContent = (postalEnabled ? postalPrice : 0).toLocaleString('fa') + ' تومان';
            return;
        }
    ...
    `;
        ...
        if (postalEnabled) {
            ...
            <tr class="postal-row">
                ...
                <td>-</td>
                <td>-</td>
                <td>${postalPrice.toLocaleString('fa')} تومان</td>
                <td>-</td>
                <td>-</td>
            </tr>
        `;
        ...
        const totalAmount = Number(data.total_amount) || items.reduce((sum, item) => sum + Number(item.total_price), 0);
        html += `
            <tr class="total-row">
                <td colspan="5">جمع کل</td>
                <td>${totalAmount.toLocaleString('fa')} تومان</td>
                <td colspan="2"></td>
            </tr>
        </tbody>
    </table>
    `;
        ...
        totalAmountDisplay.textContent = totalAmount.toLocaleString('fa') + ' تومان';
        const finalAmount = totalAmount - discount + (postalEnabled ? postalPrice : 0);
        finalAmountDisplay.textContent = finalAmount.toLocaleString('fa') + ' تومان';
        postalPriceDisplay.textContent = (postalEnabled ? postalPrice : 0).toLocaleString('fa') + ' تومان';
    }
