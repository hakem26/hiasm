...
// دیباگ سال‌ها
error_log("report_summary_admin.php: Available years (jalali): " . implode(", ", $years_jalali));
...
if ($selected_year_jalali && isset($year_mapping[$selected_year_jalali])) {
    ...
    // دیباگ گزارش‌ها
    if (empty($reports)) {
        error_log("report_summary_admin.php: No reports found for year_jalali $selected_year_jalali");
    } else {
        foreach ($reports as $report) {
            error_log("report_summary_admin.php: Found report: work_month_id = {$report['work_month_id']}, partner_name = {$report['partner_name']}, total_sales = {$report['total_sales']}");
        }
    }
}
?>
...
<body>
    <div class="container-fluid mt-5">
        <h5 class="card-title mb-4">گزارش خلاصه (ادمین)</h5>
