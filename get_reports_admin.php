...
try {
    ...
    // دیباگ
    if (empty($reports)) {
        error_log("get_reports_admin.php: No reports found for year_jalali $year_jalali, work_month_id " . ($work_month_id ?? 'none') . ", user_id " . ($user_id ?? 'none'));
    } else {
        foreach ($reports as $report) {
            error_log("get_reports_admin.php: Found report: work_month_id = {$report['work_month_id']}, partner_name = {$report['partner_name']}, total_sales = {$report['total_sales']}");
        }
    }

    $html = '<table class="table table-light"><thead><tr><th>ماه کاری</th><th>نام همکاران</th><th>مجموع فروش</th><th>وضعیت</th><th>مشاهده</th></tr></thead><tbody>';
    if (empty($reports)) {
    ...
    error_log('get_reports_admin.php: Generated HTML: ' . $html);
    echo json_encode(['success' => true, 'html' => $html], JSON_UNESCAPED_UNICODE);
} catch (PDOException $e) {
    error_log("get_reports_admin.php: Database error: " . $e->getMessage());
