<ع?php // [BLOCK-HEADER-001] session_start(); if (!isset($_SESSION['user_id'])) { header("Location: index.php"); exit; }
    require_once 'db.php' ; require_once 'jdf.php' ; $full_name=$_SESSION['full_name'];
    $role=$_SESSION['role']==='admin' ? 'ادمین' : 'فروشنده' ; $gregorian_date=date('Y-m-d'); $jalali_date=jdate('Y/m/d',
    strtotime($gregorian_date)); // نام صفحه فعلی $page_name=basename($_SERVER['PHP_SELF'], ".php" );
    $page_name=$page_name==='dashboard_admin' || $page_name==='dashboard_seller' ? 'داشبورد' : $page_name;
    $page_name=$page_name==='products' ? 'محصولات' : $page_name; $page_name=$page_name==='orders' ? 'سفارشات' :
    $page_name; $page_name=$page_name==='users' ? 'کاربران' : $page_name; $page_name=$page_name==='partners' ? 'همکاران'
    : $page_name; $page_name=$page_name==='work_months' ? 'ماه کاری' : $page_name;
